...
//        return preg_match('#^13[\d]{9}$|14^[0-9]\d{8}|^15[0-9]\d{8}$|^18[0-9]\d{8}$#', $str);
        return preg_match('#^13[\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^17[0,6,7,8]{1}\d{8}$|^18[\d]{9}$#', $str);
    }
    ...
    public static function regex($value, $rule)
    {
        ...
        $validate = array(
            ...
            'english' => '/^[A-Za-z]+$/',
            'symbol' => '/^[!@#$%^&*]+$/',
            'exist_chinese'   =>  '/[^\x00-\x80]/',
//            'chinese'   =>  '/^[\x{4e00}-\x{9fa5}]+$/u',
            'chinese' => '/^[\x{4e00}-\x{9fa5}]+(·[\x{4e00}-\x{9fa5}]+)*$/u',
            'mobile' => '/^1[3|4|5|7|8][0-9]\d{8}$/', //11位手机号
            'date' => '/^([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})$/', //2016-8-22
            'idcard' => '/^[\d]{6}((19[\d]{2})|(200[0-8]))((0[1-9])|(1[0-2]))((0[1-9])|([12][\d])|(3[01]))[\d]{3}[0-9xX]$/', //身份证号
            'qq' => '/^[1-9][0-9]{5,11}$/',
            'time' => '/^([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})$/',
            'day' => '/[\d]{4}-[\d]{1,2}-[\d]{1,2}\s[\d]{1,2}:[\d]{1,2}:[\d]{1,2}/',//2016-8-22 23:59:59
            'ip' => '/^(\d{1,3}\.){3}\d{1,3}$/',
