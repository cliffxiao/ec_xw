...
class UsersModel extends CommonModel
{
    ...
    public function __construct()
    {
        ...
        $this->table = $this->pre . 'users_'.($_SESSION['user_id'] % 10);
    }
    ...
    function update_user_info() {
        /*
        // 查询会员信息
        $sql = 'select last_login,last_ip,email from ' . $this->pre . 'users where user_id = '.$_SESSION['user_id'];
     ...
     * 判断用户是否验证通过
     */
    function is_validated($userid){
        $is_validated = $this->row("select is_validated,mobile_phone from " . $this->pre . "users where user_id = '$userid'");
		if(empty($is_validated)){
			return false;
		}
		if(strlen($is_validated['mobile_phone'])!=11){
			return false;
		}
    ...
    function send_regiter_hash($user_id) {
        /* 设置验证邮件模板所需要的内容信息 */
        $template = model('Common')->get_mail_template('register_validate');
    ...
    function cancel_order($order_id, $user_id = 0) {
        /* 查询订单信息，检查状态 */
        $sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status,order_type FROM " . $this->pre . "order_info WHERE order_id = '$order_id'";
        $order = $this->row($sql);
        ...
        if ($order['pay_status'] == PS_PAYED) {
            ECTouch::err()->add(L('current_ps_payed'));
        ...
        if($order['order_type'] == 2){
            $cancel_type = 4;//二手卡手动取消
        }else{
            $cancel_type = 2; //一手卡手动取消
        }

        $sql = "UPDATE " . $this->pre . "order_info SET order_status = '" . OS_CANCELED . "',pay_status = '".PS_UNPAYED."',cancel_type = ".$cancel_type." WHERE order_id = '$order_id'";
        ...
        if ($this->query($sql)) {
            ...
            /*如果是二手卡转卖 执行回收处理*/
            if($order['order_type'] == 2){
                model('Trade')->cancelTrade($order['order_sn']);
            }
     ...
     * @return
     */
    function check_user_phone( $user_id ) {
        $sql = 'SELECT mobile_phone FROM ' . $this->pre . 'users WHERE user_id = "' . $user_id . '" ';
        $res = $this->row($sql);
