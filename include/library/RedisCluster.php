...
class RedisCluster {
	...
	public function __construct($isUseCluster=false){
		...
		try
		{
            $this->_config = C('REDIS');
			$this->connect(array('host'=>$this->_config['host'],'port'=> $this->_config['port']));
		}
		catch(Exception $e)
		{
			return $e->getMessage();
	 ...
	 * 根据通配符*批量读缓存
	 *
	 * @param string $key 某一类型缓存KEY,传入后拼接通配符进行获取
	 * @return array || boolean 失败返回 false, 成功返回批量数组
	 */
	public function gets($key){
		// 没有使用M/S
		if(! $this->_isUseCluster){
			return $this->getRedis()->mGet($this->getRedis()->keys($key."*"));
		}
		// 使用了 M/S
		return $this->_getSlaveRedis()->mGet($this->_getSlaveRedis()->keys($key."*"));
	}
	/**
	...
	public function removes($key){
		return $this->getRedis()->delete($this->getRedis()->keys($key."*"));
	}
	/**
