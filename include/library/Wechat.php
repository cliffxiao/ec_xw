...
class Wechat
{
	...
	const CARD_CREATE = 'https://api.weixin.qq.com/card/create?';//创建卡券
	const LOGO_UPLOAD = 'https://api.weixin.qq.com/cgi-bin/media/uploadimg?';
	const GET_CARD_INFO = 'https://api.weixin.qq.com/card/get?';//获取卡券信息
    const IS_OPEN_PAY = 'https://api.weixin.qq.com/card/paycell/set?';//设置买单接口
    const UPDATE_CODE = 'https://api.weixin.qq.com/card/code/update?';//更改Code接口
	const CARD_CODE_GET = 'https://api.weixin.qq.com/card/code/get?';//查询Code接口
	const CARD_CODE_CONSUME = 'https://api.weixin.qq.com/card/code/consume?';//核销Code接口
	const CARD_CODE_DECRYPT = 'https://api.weixin.qq.com/card/code/decrypt?';//Code解码接口
    const MODIFY_CARD_STOCK = 'https://api.weixin.qq.com/card/modifystock?';//更改卡券库存
    const UPDATE_CARD = 'https://api.weixin.qq.com/card/update?';//更改卡券信息
    const UPDATE_USER_CARD = 'https://api.weixin.qq.com/card/membercard/updateuser?';//更新会员信息
    const DEPOSIT_CARD_CODE = 'http://api.weixin.qq.com/card/code/deposit?';//导入CODE
    const GET_DEPOSIT_COUNT = 'http://api.weixin.qq.com/card/code/getdepositcount?';//查询导入code数目接口
    const SET_TEST_WHITE_LIST = 'https://api.weixin.qq.com/card/testwhitelist/set?';//设置白名单
	...
	*/
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
    		...
    		$redis->set('access_token', $this->access_token, $expire - 5400);
    		//end
			return $this->access_token;
		}
		return false;
	...
	/**
	 * 创建卡券
	 * @param array $data 接口所需数据
	 * @return boolean|array
	 * ref:http://mp.weixin.qq.com/wiki/11/9116b6dacf7b7ee312e29a2d523434f7.html
	 */
	public function createCard($data) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::CARD_CREATE.'access_token='.$this->access_token,self::json_encode($data));
		print_r($result);exit();
		if ($result)
		{
			$json = json_decode($result,true);
	 ...
	 * @param array $data 接口所需数据
	 * @return boolean|array
	 * ref:http://mp.weixin.qq.com/wiki/19/a47c9dcb4c6e03fcdd478bf6803d59c3.html
	 */
	public function uploadLogo($data) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::LOGO_UPLOAD.'access_token='.$this->access_token,$data);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	public function getCardInfo($card_id) {
		...
		$result = $this->http_post(self::GET_CARD_INFO.'access_token='.$this->access_token,self::json_encode($data));
		print_r($result);exit();
		if ($result)
		{
			$json = json_decode($result,true);
    ...
    * 卡片核销
    */
    public  function cardConsume($card_id)
    {
        $card_info = model('Goods')->get_goods_join_virtual_card($card_id); //获取卡片的信息
        if ($card_info) {
            $card_id = $card_info['card_id'];
            $vc_card_id = $card_info['vc_card_id'];
        } else {
            return '卡片核销（获取卡片的信息）：卡片信息错误';
        }
        /*这块是赠送好友之后必须更新的数据    更新卡券的card_sn_wx*/
        $card_sn_wx = $card_info['card_sn_wx']; //获取card_sn_wx
        $wx_code = intval(substr($card_sn_wx, -4)) + 1; //获取转赠次数
        if ($wx_code < 10) {
            $wx_code = '000' . $wx_code;
        } elseif ($wx_code < 100) {
            $wx_code = '00' . $wx_code;
            ...
            $wx_code = '0' . $wx_code;
        } elseif ($wx_code < 10000) {
            $wx_code = $wx_code;
        } else {
            return '核销卡片：用户转赠次数太多。';
        }
        model('Couponorder')->update_virtual_card_by_card_id($card_id . $wx_code, 0, $vc_card_id);
        ...
        $result = $this->cardCodeGet($data); //查询Code接口
        if ($result['errmsg'] == 'ok' && $result['can_consume'] == true) { //返回可以核销
            unset($data['check_consume']); //去除 check_consume
            $res = $this->cardCodeConsume($data); //核销卡片
        } elseif ($result['errcode'] == '40127') {
            return '核销卡券前的查询失败。';
        }
        return '核销成功';
    }
	...
	/**
	 * 查询Code接口
	 * @param array $cardInfo  card_id：卡券ID代表一类卡券  code：单张卡券的唯一标准。 check_consume：是否校验code核销状态
	 * @return boolean|array
	 * ref:http://mp.weixin.qq.com/wiki/11/9116b6dacf7b7ee312e29a2d523434f7.html
	 */
	public function cardCodeGet($cardInfo) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::CARD_CODE_GET.'access_token='.$this->access_token,self::json_encode($cardInfo));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	/**
	 * 核销Code接口
	 * @param array $data  card_id：卡券ID代表一类卡券  code：单张卡券的唯一标准。
	 * @return boolean|array
	 * ref:http://mp.weixin.qq.com/wiki/11/9116b6dacf7b7ee312e29a2d523434f7.html
	 */
	public function cardCodeConsume($data) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::CARD_CODE_CONSUME.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	/**
	 * Code解码接口
	 * @param array $data  encrypt_code：卡券ID代表一类卡券。
	 * @return boolean|array
	 * ref:http://mp.weixin.qq.com/wiki/11/9116b6dacf7b7ee312e29a2d523434f7.html
	 */
	public function cardCodeDecrypt($data) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::CARD_CODE_DECRYPT.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 * 卡券H5投放-获取签名包
	 */
	public function GetSignPackage($timenow=0,$url = '') {
	...
	private function getJsApiTicket() {
		...
		if(!$redis->get('jsapi_ticket')){
    		...
    		if($json['ticket']){
    			$expire = $json['expires_in'] ? intval($json['expires_in']) : 7200;
    			$redis->set('jsapi_ticket', $json['ticket'], $expire - 5400);
    		}
		}
	...
	private function getApiTicket() {
		...
		if (!$redis->get('api_ticket')) {
    		...
    		if($json['ticket']){
    			$expire = $json['expires_in'] ? intval($json['expires_in']) : 7200;
    			$redis->set('api_ticket', $json['ticket'], $expire - 5400);
    		}
		}
		return $redis->get('api_ticket');
	}
}